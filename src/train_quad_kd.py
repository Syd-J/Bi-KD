...
parser = argparse.ArgumentParser(description='Quad-KD Training (4 models)')
...
parser.add_argument('--model_4',type=str,required=True)
...
parser.add_argument('--lr_4',default=1e-6,type=float,help='learning rate for model 4')
...
parser.add_argument('--load_4_path',default='',type=str,help='path to load model 4')
parser.add_argument('--model_1_val_path',default='best_quad_kd_1.pt',type=str,help='path to save the best model 1')
parser.add_argument('--model_2_val_path',default='best_quad_kd_2.pt',type=str,help='path to save the best model 2')
parser.add_argument('--model_3_val_path',default='best_quad_kd_3.pt',type=str,help='path to save the best model 3')
parser.add_argument('--model_4_val_path',default='best_quad_kd_4.pt',type=str,help='path to save the best model 4')
...
model_4_params = list(filter(lambda p: p.requires_grad, model_4.parameters()))
...
model_4_optimizer = torch.optim.Adam(model_4_params, lr=args.lr_4, weight_decay=args.weight_decay)
...
best_4_acc = load_model_checkpoint(model_4, model_4_optimizer, args.load_4_path, metric_type='acc')

criterion = nn.CrossEntropyLoss()

def train(model_1, model_2, model_3, model_4, model_1_optimizer, model_2_optimizer, model_3_optimizer, model_4_optimizer, train_loader, epoch, device, args):
	"""Train four models using quad-directional knowledge distillation."""
	model_1.train()
	model_2.train()
	model_3.train()
	model_4.train()
	
	correct_1, correct_2, correct_3, correct_4, total = 0, 0, 0, 0, 0
	curr_loss = AverageMeter()
	total_loss = AverageMeter()
	curr_ce_1_loss = AverageMeter()
	curr_ce_2_loss = AverageMeter()
	curr_ce_3_loss = AverageMeter()
	curr_ce_4_loss = AverageMeter()
	total_ce_1_loss = AverageMeter()
	total_ce_2_loss = AverageMeter()
	total_ce_3_loss = AverageMeter()
	total_ce_4_loss = AverageMeter()
	tic = time.time()
	...
	for idx, sample in enumerate(train_loader):
		...
		pred_4 = get_model_output(model_4, imgs, args.model_4)
		...
		gt_soft_4 = pred_4.softmax(dim=1)[torch.arange(args.batch_size), labels]
		
		# Stack softmax values and find which model is most confident
		gt_soft_stack = torch.stack((gt_soft_1, gt_soft_2, gt_soft_3, gt_soft_4), dim=1)
		...
		ce_4 = criterion(pred_4, labels)

		# KL divergence losses: when one model teaches, the other three learn from it
		kl_1 = F.kl_div(soft_2.log(), soft_1, reduction='none').sum(dim=1) + F.kl_div(soft_3.log(), soft_1, reduction='none').sum(dim=1) + F.kl_div(soft_4.log(), soft_1, reduction='none').sum(dim=1)
		kl_2 = F.kl_div(soft_1.log(), soft_2, reduction='none').sum(dim=1) + F.kl_div(soft_3.log(), soft_2, reduction='none').sum(dim=1) + F.kl_div(soft_4.log(), soft_2, reduction='none').sum(dim=1)
		kl_3 = F.kl_div(soft_1.log(), soft_3, reduction='none').sum(dim=1) + F.kl_div(soft_2.log(), soft_3, reduction='none').sum(dim=1) + F.kl_div(soft_4.log(), soft_3, reduction='none').sum(dim=1)
		kl_4 = F.kl_div(soft_1.log(), soft_4, reduction='none').sum(dim=1) + F.kl_div(soft_2.log(), soft_4, reduction='none').sum(dim=1) + F.kl_div(soft_3.log(), soft_4, reduction='none').sum(dim=1)

		# Total loss: CE losses + masked quad-directional KL divergence
		loss = ce_1 + ce_2 + ce_3 + ce_4 + (mask_1 * kl_1 + mask_2 * kl_2 + mask_3 * kl_3 + mask_4 * kl_4).mean()

		loss.backward()
		model_1_optimizer.step()
		model_2_optimizer.step()
		model_3_optimizer.step()
		model_4_optimizer.step()
		...
		correct_4 += (soft_4.argmax(dim=1) == labels).sum().item()
		total += labels.size(0)
		...
		curr_ce_4_loss.update(ce_4.item())
		total_ce_1_loss.update(ce_1.item())
		total_ce_2_loss.update(ce_2.item())
		total_ce_3_loss.update(ce_3.item())
		total_ce_4_loss.update(ce_4.item())

		# Log training progress at intervals
		if idx%args.log_interval==(args.log_interval-1):
			print('[{:2d}, {:5d}/{:5d}] avg_loss : {:.3f}, ce_1 : {:.3f}, ce_2 : {:.3f}, ce_3 : {:.3f}, ce_4 : {:.3f}, 1_acc : {:.2f}, 2_acc : {:.2f}, 3_acc : {:.2f}, 4_acc : {:.2f}, time: {:.3f} minutes'.format(epoch+1, idx+1, len(train_loader), curr_loss.avg, curr_ce_1_loss.avg, curr_ce_2_loss.avg, curr_ce_3_loss.avg, curr_ce_4_loss.avg, (100 * correct_1) / total, (100 * correct_2) / total, (100 * correct_3) / total, (100 * correct_4) / total, (time.time()-tic)/60))
			curr_loss.reset()
			curr_ce_1_loss.reset()
			curr_ce_2_loss.reset()
			curr_ce_3_loss.reset()
			curr_ce_4_loss.reset()
			sys.stdout.flush()
	...
	if args.log_wandb:
		wandb.log({'train_avg_loss': total_loss.avg, 'train_ce_1': total_ce_1_loss.avg, 'train_ce_2': total_ce_2_loss.avg, 'train_ce_3': total_ce_3_loss.avg, 'train_ce_4': total_ce_4_loss.avg, 'train_1_acc': ((100 * correct_1) / total), 'train_2_acc': ((100 * correct_2) / total), 'train_3_acc': ((100 * correct_3) / total), 'train_4_acc': ((100 * correct_4) / total)}, step=epoch+1)
	
	print('[{:2d}, train] avg_loss : {:.3f}, ce_1 : {:.3f}, ce_2 : {:.3f}, ce_3 : {:.3f}, ce_4 : {:.3f}, 1_acc : {:.2f}, 2_acc : {:.2f}, 3_acc : {:.2f}, 4_acc : {:.2f}, time : {:.3f} minutes'.format(epoch+1, total_loss.avg, total_ce_1_loss.avg, total_ce_2_loss.avg, total_ce_3_loss.avg, total_ce_4_loss.avg, ((100 * correct_1) / total), ((100 * correct_2) / total), ((100 * correct_3) / total), ((100 * correct_4) / total), time_taken))
	sys.stdout.flush()

	return total_loss.avg

def validate(model_1, model_2, model_3, model_4, val_loader, epoch, device):
	"""Validate all four models on the validation set."""
	model_1.eval()
	model_2.eval()
	model_3.eval()
	model_4.eval()
	...
	for sample in val_loader:
		...
		output_4 = get_model_output(model_4, img, args.model_4)
		...
		correct_4 += topk_accuracy(output_4, label, k=1)
	...
	val_4_acc = (100 * correct_4) / total
	
	# Log to wandb if enabled
	if args.log_wandb:
		wandb.log({'val_1_acc': val_1_acc, 'val_2_acc': val_2_acc, 'val_3_acc': val_3_acc, 'val_4_acc': val_4_acc}, step=epoch+1)
	
	print("Accuracy on the ImageNet validation set: {:.2f}% (model 1), {:.2f}% (model 2), {:.2f}% (model 3), {:.2f}% (model 4), time : {:.3f} minutes".format(val_1_acc, val_2_acc, val_3_acc, val_4_acc, (time.time() - tic) / 60))
	return val_1_acc, val_2_acc, val_3_acc, val_4_acc

# Training loop
for epoch in range(args.no_epochs):
	train_loss = train(model_1, model_2, model_3, model_4, model_1_optimizer, model_2_optimizer, model_3_optimizer, model_4_optimizer, train_loader, epoch, device, args)

	with torch.no_grad():
		val_1_acc, val_2_acc, val_3_acc, val_4_acc = validate(model_1, model_2, model_3, model_4, val_loader, epoch, device)
		...
		# Save model 4 checkpoint if improved
		if val_4_acc >= best_4_acc:
			best_4_acc = val_4_acc
			save_checkpoint(model_4, model_4_optimizer, best_4_acc, args.model_4_val_path, "model 4", epoch, metric_type='acc')
	print()
